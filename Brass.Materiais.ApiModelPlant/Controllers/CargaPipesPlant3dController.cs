...
namespace Brass.Materiais.ApiModelPlant.Controllers
{
    [EnableCors("MyPolicy")]
    ...
    public class CargaPipesPlant3dController : ControllerBase
    {
        ...
        [HttpPost]
        public async Task<ActionResult> CargaPipesPlant3d([FromBody] IdResumoCorrente idResumo)
        {
           
            switch (idResumo.Origem)
            {

                case "VPN":
                    {
                        var command = new CarregarItensPQPipeCommand(idResumo);
                        await _mediator.Send(command);
                        return Ok();
                    }
                case "BIM360":
                    {
                        var command = new CargaItensP3DBIM360Command(idResumo);
                        await _mediator.Send(command);
                        return Ok();
                    }
                default:
