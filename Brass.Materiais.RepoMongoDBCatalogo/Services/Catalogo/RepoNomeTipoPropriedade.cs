...
{
    public class RepoNomeTipoPropriedade : RepositorioAbstratoGeral
    {

        BaseMDBRepositorio<NomeTipoPropriedade> _nomeTipoPropriedadeRepositorio;

        public RepoNomeTipoPropriedade(string conectionString) : base(conectionString)
        {
        ...
        public NomeTipoPropriedade ObterPorNome(string nome)
        {
            var nomesTiposPropriedade = _nomeTipoPropriedadeRepositorio.Encontrar(
            Builders<NomeTipoPropriedade>.Filter.Eq(x => x.NOME, nome));

            return nomesTiposPropriedade.Count() == 1 ? nomesTiposPropriedade.First() : null;

        }

        public void CadastrarNomeTipoPropriedade(NomeTipoPropriedade nomeTipoPropriedade)
        {
